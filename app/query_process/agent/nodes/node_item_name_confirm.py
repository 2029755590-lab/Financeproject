...
def step_6_check_confirmation(state, align_result, session_id, history_list, rewritten_query):
    ...
    if options:
        ...
        # 拼接待确认信息
        answer = f"您是想问以下哪个金融产品或资料：{options_str}？请补充一下名称或代码。"
# 兜底（仅用于"提取到实体但没对齐"的情况）
        # 更新状态
        state["item_names"] = []
        state["answer"] = answer
        return state
    # 分支3：既没有已确认也没有待确认
    # 拼接待确认信息
    answer = "抱歉，未找到相关金融产品或资料，请提供准确的名称或代码，以便我为您查询。"
    # 更新状态
    state["item_names"] = []
    state["answer"] = answer
    return state
...
def node_item_name_confirm(state: QueryGraphState):
    ...
    if len(item_names) > 0:
        ...
        state["is_no_entity"] = False

    else:
        # ===== ★无实体：概念/风险/业务类问题 =====
        # 不反问、不写兜底 answer，直接让流程继续走「全库检索」
        logger.info("未提取到金融主体，进入全库检索模式（is_no_entity=True）")
        state["item_names"] = []
        state["is_no_entity"] = True
        state["question_type"] = classify_question(original_query)
        # 关键：这里绝不能给 state["answer"] 赋值，否则图会跳到答案输出
...
def classify_question(query: str) -> str:
    """粗分问题类型，便于后续按类型调整话术（可选）"""
    if any(k in query for k in ["什么是", "是什么意思", "含义", "解释"]):
        return "概念型"
    if any(k in query for k in ["风险", "亏", "安全", "保本", "收益"]):
        return "风险型"
    if any(k in query for k in ["如何", "怎么", "多久", "流程", "到账", "手续费"]):
        return "业务型"
    return "实体型"

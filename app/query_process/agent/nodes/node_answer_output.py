...
from app.conf.finance_constants import FORBIDDEN_PHRASES, NO_DATA_ANSWER
...
def step_2_construct_prompt(state: QueryGraphState):
    ...
    for num, chunk in enumerate(reranked_docs, start=1):
        ...
        # 金融新增：把元数据一并喂给大模型，用于生成"引用来源"
        if chunk.get("item_name"):
            data_list.append(f"[金融主体={chunk['item_name']}]")
        if chunk.get("content_type"):
            data_list.append(f"[内容类型={chunk['content_type']}]")
        if chunk.get("risk_level"):
            data_list.append(f"[风险等级={chunk['risk_level']}]")
        if chunk.get("publish_date"):
            data_list.append(f"[发布时间={chunk['publish_date']}]")
        if chunk.get("file_title"):
            data_list.append(f"[来源文件={chunk['file_title']}]")
...
# 新增：合规后置校验：命中禁用词则追加风险提示声明
def sanitize_answer(answer: str) -> str:
    """合规后置校验：命中禁用词则追加风险提示声明"""
    hit = [w for w in FORBIDDEN_PHRASES if w in answer]
    if hit:
        logger.warning(f"回答命中合规禁用词：{hit}，已追加风险提示")
        answer += ("\n\n【风险提示】金融产品存在风险，历史业绩不代表未来表现，"
                   "投资需结合自身风险承受能力；本回答仅供知识参考，不构成投资建议。")
    return answer
...
def node_answer_output(state: QueryGraphState):
    ...
    if not answer_exists:
        # ★无资料兜底：检索结果为空时，直接返回固定话术，不调用 LLM，杜绝编造
        if not (state.get("reranked_docs") or []):
            state["answer"] = NO_DATA_ANSWER
            if state.get("is_stream"):
                push_to_session(state["session_id"], SSEEvent.DELTA, {"delta": NO_DATA_ANSWER})
        else:
    ...
    if state.get("answer"):
        ...
        state["answer"] = sanitize_answer(original)
        # ★非流式：必须把合规后的答案重新写回任务结果
        # 原因：step_3_generate_response 的非流式分支里已经 set_task_result 存了"未合规"的原文，
        #       而 query_service.py 的 /query 接口取回的正是它 → 不覆盖的话 sanitize 等于白做。
        if not state.get("is_stream"):
            set_task_result(state["session_id"], "answer", state["answer"])
        # ★（可选）流式：把"追加的部分"也作为增量推出去，避免末尾整段跳变
        added = state["answer"][len(original):]
        if added and state.get("is_stream"):
            push_to_session(state["session_id"], SSEEvent.DELTA, {"delta": added})

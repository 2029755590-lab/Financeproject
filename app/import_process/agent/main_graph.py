# 加载环境变量：从 .env 文件读取配置（如Mil vus地址、KG服务地址、BGE模型路径等）
...
from app.import_process.agent.nodes.node_docx_to_md import node_docx_to_md  # DOCX转MD：将docx文件转换为markdown格式
...
from app.import_process.agent.nodes.node_bge_embedding import node_bge_embedding  # BGE向量化：将文本分块转换为向量表示（适配Mil vus向量库）
from app.import_process.agent.nodes.node_import_milvus import node_import_milvus  # 导入Mil vus：将向量数据写入Mil vus向量数据库
...
workflow.add_node("node_bge_embedding", node_bge_embedding)  # BGE向量化：文本→向量，为Mil vus存储做准备
workflow.add_node("node_import_milvus", node_import_milvus)  # 向量入库：将向量数据持久化到Mil vus
#新增节点：node_docx_to_md（将docx文件转换为markdown格式）
...
# 新增边：将docx转MD完成后的状态传递给MD图片处理节点
workflow.add_edge("node_docx_to_md", "node_md_img")  # docx转MD完成 → MD图片处理
...
workflow.add_edge("node_bge_embedding", "node_import_milvus")  # 向量化完成 → 导入Mil vus向量库
workflow.add_edge("node_import_milvus", END)  # Mil vus入库完成 → 工作流执行结束（END是内置结束节点）

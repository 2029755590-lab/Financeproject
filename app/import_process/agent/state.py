...
class ImportGraphState(TypedDict):
    ...
    local_file_path: str  # 原始输入文件路径   上传服务器后的地址
    file_title: str       # 文件标题（文件名去后缀）
    pdf_path: str         # PDF 文件路径 (如果输入是PDF)
    md_path: str          # Markdown 文件路径 (转换后或直接输入的)
    docx_path: str        # docx 文件路径 (如果输入是docx， ★识别出的金融主体（产品名/公司名/资料名）)

    # --- 内容数据 ---
    md_content: str       # Markdown 的全文内容
    chunks: list          # 切片后的文本列表，包含 metadata
    item_name: str        # 识别出的主体名称 (如: 金融主体)，用于增强检索

    # --- 数据库相关 ---
    embeddings_content: list # 包含向量数据的列表，准备写入 Mil vus
...
if __name__ == "__main__":
    ...
    """
    # 创建默认状态
    state = create_default_state(local_file_path="百济神洲公司年报.pdf")
    logger.info(state)
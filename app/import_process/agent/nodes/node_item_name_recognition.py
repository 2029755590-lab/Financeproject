...
def step_4_update_chunks_and_state(state, item_name, chunks):
    ...
    # 获取文件标题
    source_file = state.get("file_title", "")
    # 更新每个切片，添加item_name信息
    for chunk in chunks:
        chunk["item_name"] = item_name

        # ★金融新增：动态字段（Milvus 已开 enable_dynamic_field，可直接写）
        chunk["product_name"] = item_name  # 产品/机构名
        chunk["content_type"] = detect_content_type(source_file, chunk.get("title", ""))
        chunk["risk_level"] = detect_risk_level(chunk.get("content", ""))  # R1~R5，识别不到为空
        chunk["publish_date"] = detect_publish_date(chunk.get("content", ""))
        chunk["source_file"] = source_file
    # 更新状态中的chunks
    state["chunks"] = chunks

# 功能：按文件名/标题粗判内容类型（金融需求字段：content_type）
def detect_content_type(source_file: str, title: str) -> str:
    """按文件名/标题粗判内容类型（金融需求字段：content_type）"""
    text = f"{source_file} {title}"
    mapping = [
        (["风险揭示", "风险提示"], "风险提示"),
        (["产品资料概要", "说明书", "招募"], "产品说明书"),
        (["季报", "年报", "公告", "报告"], "公告摘要"),
        (["FAQ", "问答", "投资者教育"], "FAQ"),
        (["统计公报", "货币政策", "办法", "政策"], "市场资讯"),
    ]
...
# 功能：从正文中提取风险等级（如 R2、R3）
def detect_risk_level(content: str) -> str:
    """从正文中提取风险等级（如 R2、R3）"""
    import re
    m = re.search(r"[Rr]\s?([1-5])", content)
    return f"R{m.group(1)}" if m else ""

# 功能：从正文中提取日期（如 2026-04-25 / 2026年4月25日）
def detect_publish_date(content: str) -> str:
    """从正文中提取日期（如 2026-04-25 / 2026年4月25日）"""
    import re

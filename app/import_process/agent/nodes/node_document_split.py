...
CHUNK_SIZE = 500 # 小值方便测试切割
# 块之间重叠长度（保证语义不丢失）
CHUNK_OVERLAP = 50

# FAQ 问答对的起始标记：一、/ 1. / 1、/ 问：
FAQ_START_PATTERN = re.compile(r"^\s*(?:[一二三四五六七八九十百]+[、.．]|[0-9]{1,3}[、.．]|问[:：])")

def _is_faq_document(file_title: str, md_content: str) -> bool:
    """判断是否 FAQ 文档：文件名含 FAQ/问答/知识问答，或正文里有 3 处以上问答标号"""
    if any(k in file_title for k in ["FAQ", "问答", "faq"]):
        return True
    hits = len(FAQ_START_PATTERN.findall(md_content))
    return hits >= 3


def split_faq_by_qa(md_content: str, file_title: str):
    """FAQ 文档：按「一问一答」切成独立块，让每个问答对成为完整检索单元"""
    ...
    for line in lines:
        if FAQ_START_PATTERN.match(line):
            if current_title:
                qa_blocks.append({"title": current_title, "content": "\n".join(current_lines).strip()})
    ...
    # 兜底：没切出任何块，按整篇处理
    if not qa_blocks:
        qa_blocks.append({"title": file_title, "content": md_content})

    # 统一补上 file_title
    for block in qa_blocks:
        block["file_title"] = file_title
        block["parent_title"] = file_title
    return qa_blocks
...
def step_2_split_by_title(md_content, file_title):
    ...
    # 新增：判断是否是 FAQ 文档
    if _is_faq_document(file_title, md_content):
        # 如果是 FAQ 文档，按「一问一答」切分
        logger.info(f"检测到 FAQ 文档 {file_title}")
        return split_faq_by_qa(md_content, file_title)
...
def step_3_refine_chunks(sections):
    ...
    for chunk in sections:
        ...
        for idx, sub_chunk in enumerate(sub_chunks, start=1):
            # 获取当前子切片的标题
            # 若有多个子切片，current_title=title_1,title_2,title_3...
...
def node_document_split(state: ImportGraphState) -> ImportGraphState:
    ...
    # 步骤3：使用RecursiveCharacterTextSplitter进行二次切分，控制切片大小
